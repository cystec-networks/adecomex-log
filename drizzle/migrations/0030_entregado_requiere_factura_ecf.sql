...
DECLARE
  ...
  tiene_fac boolean; tiene_bl boolean; falta_docs text; es_priv boolean;
BEGIN
  ...
  FOR i IN (i_old + 1)..i_new LOOP
    ...
    ELSIF paso = 'despachado' AND NEW.fecha_aprobacion_despacho IS NULL THEN
      RAISE EXCEPTION 'No se puede despachar este Expediente: falta capturar la Fecha de Aprobación del N° de despacho.';
    ...
    IF paso = 'despachado' THEN
      ...
      END IF;
    END IF;
    IF paso = 'entregado' AND NEW.factura_ecf_id IS NULL THEN
      IF es_priv THEN
        INSERT INTO public.auditoria (entidad, entidad_id, accion, usuario_id, cambios)
        VALUES ('expedientes', NEW.id, 'entregado_forzado_sin_ecf:' || OLD.estado::text || '->' || NEW.estado::text, auth.uid(),
                jsonb_build_object('motivo', NEW.motivo_regreso_estado));
      ELSE
        RAISE EXCEPTION 'No se puede marcar como Entregado este Expediente: falta vincular la Factura E-CF (DGII).';
      END IF;
    END IF;
  END LOOP;
  RETURN NEW;
