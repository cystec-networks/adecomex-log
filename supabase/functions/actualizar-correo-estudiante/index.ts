...
Deno.serve(async (req) => {
  ...
  let payload: { estudiante_id?: string; nuevo_correo?: string };
  ...
  if (!estudiante_id || !nuevo_correo) return json(400, { error: "estudiante_id y nuevo_correo son requeridos" });
  ...
  if (link?.user_id) {
    ...
    if (authErr) {
      warning = `El correo se actualizó en la ficha del estudiante, pero no se pudo sincronizar con la cuenta de acceso (${authErr.message}).`;
    }
  }
