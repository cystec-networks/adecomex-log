...
import {
  extractCuentaPorPagarFromDocument,
  type CuentaPorPagarExtraction,
} from "@/lib/ai-ocr-cxp.functions";

export function EscanearFacturaCxpButton({
  onExtracted,
  size = "sm",
}: {
  onExtracted: (data: CuentaPorPagarExtraction) => void;
  size?: "sm" | "default";
}) {
  const extractFn = useServerFn(extractCuentaPorPagarFromDocument);
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const [data, setData] = useState<CuentaPorPagarExtraction | null>(null);
            ...
            {data && (
              <div className="border rounded-md p-3 bg-muted/20 space-y-2 text-sm">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                  ...
                  <div><span className="text-muted-foreground">RNC:</span> {fmt(data.proveedor_rnc)}</div>
                  <div><span className="text-muted-foreground">No. factura:</span> {fmt(data.numero_factura)}</div>
                  <div><span className="text-muted-foreground">NCF:</span> {fmt(data.ncf_proveedor)}</div>
                  <div><span className="text-muted-foreground">Fecha factura:</span> {fmt(data.fecha_factura)}</div>
