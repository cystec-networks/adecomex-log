...
export const Route = createFileRoute("/_portal-estudiante/cambiar-password")({
  ...
  component: CambiarPasswordEstudiante,
});
...
function CambiarPasswordEstudiante() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const { error: rpcErr } = await (supabase as any).rpc("marcar_password_cambiada_estudiante");
    setLoading(false);
    if (rpcErr) return toast.error(rpcErr.message);
    toast.success("Contraseña actualizada");
    navigate({ to: "/portal-estudiante", replace: true });
  };
