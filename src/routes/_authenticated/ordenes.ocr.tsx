...
function OrdenesOCR() {
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const { data: o, error } = await supabase.from("ordenes").insert(payload).select().single();
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link to="/ordenes"><ArrowLeft className="h-4 w-4 mr-1" />Volver</Link>
        </Button>
        <div>
          <h1 className="font-display text-2xl font-bold flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-accent" /> OCR de documentos — Orden
          </h1>
      ...
      {data && (
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            ...
            <div className="grid gap-1.5 md:col-span-2">
              <Label>Notas (se guardarán en la orden)</Label>
              <Textarea rows={5} value={notas} onChange={(e) => setNotas(e.target.value)} />
            </div>
            <p className="md:col-span-2 text-xs text-muted-foreground">
              Tipo de mercancía, origen, destino, incoterm, peso y volumen se completan manualmente en la ficha de la orden.
            </p>
            <div className="md:col-span-2 flex justify-end gap-2">
              <Button variant="outline" onClick={() => { setData(null); setFile(null); }}>Descartar</Button>
              <Button onClick={() => create.mutate()} disabled={create.isPending}>
                {create.isPending ? "Creando…" : "Crear borrador de orden"}
              </Button>
            </div>
          </CardContent>
        </Card>
