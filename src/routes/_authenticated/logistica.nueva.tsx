...
function NuevaOperacion() {
  ...
  const LinkSelect = ({ label, field, rows }: { label: string; field: "cotizacion_id" | "orden_id" | "expediente_id"; rows: { id: string; numero: string }[] }) => <div className="space-y-1.5"><Label>{label}</Label><Select value={form[field] || "none"} onValueChange={(v) => set(field, v === "none" ? "" : v)}><SelectTrigger><SelectValue placeholder="Sin vincular" /></SelectTrigger><SelectContent><SelectItem value="none">Sin vincular</SelectItem>{rows.map((r) => <SelectItem key={r.id} value={r.id}>{r.numero}</SelectItem>)}</SelectContent></Select></div>;
